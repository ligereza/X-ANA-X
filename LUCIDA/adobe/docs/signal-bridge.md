...
The bridge never forwards raw text, source files, image data, paths, keyboard values, scripts, shell commands, URLs or arbitrary payloads. Signals do not execute host actions. `visual` and `pupila` proposals are confirmation-only and proposal-only.
