...
Esta carpeta es una referencia portable del frente IRIS incluida dentro del
experimento investigativo 093 de FARMAKSIA. No es un runtime importado por
FARMAKSIA ni una promesa de integración productiva. Los directorios
`node_modules/`, `outputs/` y `work/` se mantienen fuera del repositorio.

Los fixtures `mak-*-sample.json` contienen un subconjunto de metadatos
observados en MAK y sus manifests registran la procedencia y el hash. Se
conservan los IDs estables para poder rastrear el origen; no se copian medios
privados. La demo y el medio SVG de prueba son sintéticos.
